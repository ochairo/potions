...
func (d *Downloader) extractTarGz(tarPath, destDir string) error {
	...
	for {
		...
		case tar.TypeReg:
			...
			// Create file with writable permissions (ensure we can delete it later)
			// Preserve execute bit if present, but ensure owner can read/write
			// Validate mode to prevent integer overflow
			var mode os.FileMode
			if header.Mode >= 0 && header.Mode <= 0777 {
				mode = os.FileMode(header.Mode)
				// Ensure at minimum 0600 (owner read/write) and preserve execute bits
				if mode&0100 != 0 {
					// If original had execute bit, preserve it with 0755
					mode = 0755
				} else {
					// Otherwise use 0644 (readable by all, writable by owner)
					mode = 0644
				}
			} else {
			...
			outFile, err := os.OpenFile(target, os.O_CREATE|os.O_RDWR, mode)
			if err != nil {
				return fmt.Errorf("failed to create file: %w", err)
			}
...
func (d *Downloader) cloneGitRepo(gitURL, tag, destDir string) error {
	...
	if strings.Contains(gitURL, "--") || strings.Contains(tag, "--") {
		return fmt.Errorf("invalid git URL or tag: contains potential command injection")
	}
	if strings.ContainsAny(gitURL, ";&|`$") || strings.ContainsAny(tag, ";&|`$") {
		return fmt.Errorf("invalid git URL or tag: contains shell metacharacters")
	}

	//nolint:gosec // G204: gitURL and tag validated above
