...
func TestCLI_Monitor(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:         []string{"monitor", "--recipes-dir", recipesDir, "kubectl", "helm", "age", "jq", "yq"},
			...
			validate: func(t *testing.T, output string) {
				...
				// Should have checked the specified packages
				if len(results) < 3 {
					t.Errorf("Expected to check at least 3 packages, got %d", len(results))
				}
			},
		},
		{
