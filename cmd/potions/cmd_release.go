...
`)
	...
	if *packages != "" {
		...
		if err := releaseFromPackageList(ctx, *packages, *artifactsDir, *recipesDir, *owner, *repo, token, *reportFile, *failuresFile, *successesFile); err != nil {
...
func releaseFromPackageList(ctx context.Context, packagesJSON, artifactsDir, recipesDir, owner, repo, token, reportFile, failuresFile, successesFile string) error {
	...
	for i, pkg := range packages {
		fmt.Printf("[%d/%d] Processing %s v%s\n", i+1, len(packages), pkg.Package, pkg.Version)
...
func fetchExistingReleases(_ context.Context, _ *gateways.HTTPGitHubGateway, _, _ string) (map[string]bool, error) {
	// Note: This would need to be implemented in the GitHub gateway
	// For now, return empty map to avoid blocking
	// TODO: Add ListReleases method to GitHub gateway
