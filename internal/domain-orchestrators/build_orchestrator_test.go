...
func TestBuildOrchestrator_BuildPackage_Success(t *testing.T) {
	...
	result, err := orch.BuildPackage(context.Background(), "kubectl", "1.28.0", "linux-amd64")
...
func TestBuildOrchestrator_DownloadFailure(t *testing.T) {
	recipe := &entities.Recipe{
		Name: "kubectl",
		Download: entities.RecipeDownload{
			Platforms: map[string]entities.PlatformConfig{
	...
	orch := NewBuildOrchestrator(
		...
		nil,
		&mockVersionFetcher{version: "1.0.0"},
		&mockDownloader{err: errors.New("network timeout")},
		&mockScriptExecutor{},
		&mockPackager{},
		BuildOrchestratorConfig{},
	)
...
func TestBuildOrchestrator_BuildScriptFailure(t *testing.T) {
	...
	orch := NewBuildOrchestrator(
		...
		&mockScriptExecutor{err: errors.New("compilation failed")},
		&mockPackager{},
		BuildOrchestratorConfig{},
	)
...
func TestBuildOrchestrator_PackagingFailure(t *testing.T) {
	recipe := &entities.Recipe{
		Name: "kubectl",
		Download: entities.RecipeDownload{
			Platforms: map[string]entities.PlatformConfig{
	...
	orch := NewBuildOrchestrator(
		...
		&mockDownloader{artifact: &entities.Artifact{}},
		&mockScriptExecutor{},
		&mockPackager{err: errors.New("tar creation failed")},
		BuildOrchestratorConfig{},
	)
	...
	if err == nil {
		t.Fatal("Expected error for packaging failure, got nil")
	}

	if !contains(err.Error(), "packaging failed") {
		t.Errorf("Error message = %v, want 'packaging failed'", err.Error())
	}
}
