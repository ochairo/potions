...
func TestCLI_Monitor(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctx, cancel := context.WithTimeout(context.Background(), 120*1e9) // 2 min
			defer cancel()
...
func TestCLI_ValidateRelease(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: []string{
				"validate-release", packageName, "latest",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectFail {
				if err != nil && !strings.Contains(string(output), "Missing platforms") {
					t.Errorf("Expected 'Missing platforms' error, got: %v", err)
				}
			} else if err != nil {
