...
const (
	// maxRecipeFileSize prevents excessive memory usage (1MB limit)
	maxRecipeFileSize = 1 * 1024 * 1024
)

// validateRecipeName ensures recipe names don't contain path traversal
func validateRecipeName(name string) error {
	...
	if !matched {
		return fmt.Errorf("invalid recipe name: only alphanumeric, hyphen, and underscore allowed")
	}

	// Additional safety: block obviously bad patterns
	if strings.Contains(name, "..") || strings.Contains(name, "/") || strings.Contains(name, "\\") {
		return fmt.Errorf("recipe name contains path traversal characters")
	}
...
func (r *RecipeRepository) GetRecipe(_ context.Context, name string) (*entities.Recipe, error) {
	// SECURITY: Validate recipe name to prevent path traversal
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to stat recipe file: %w", err)
	}
	if stat.Size() > maxRecipeFileSize {
		return nil, fmt.Errorf("recipe file too large: %d bytes (max: %d)", stat.Size(), maxRecipeFileSize)
	}
